...
final class CoreTests: XCTestCase {
    ...
    func testTaskLightEffectsAreIndependentAndCodable() throws {
        var effects = CodexTaskLightEffects()
        effects.setEffect(.init(animation: .blink, speed: .slow), for: .reasoning)
        XCTAssertEqual(effects.effect(for: .reasoning), .init(animation: .blink, speed: .slow))
        XCTAssertEqual(effects.effect(for: .error), .init(animation: .blink, speed: .fast))
        XCTAssertEqual(try JSONDecoder().decode(CodexTaskLightEffects.self,
                                                from: JSONEncoder().encode(effects)), effects)
    }
